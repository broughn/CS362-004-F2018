...
int adventurerEffect(struct gameState *state, int drawntreasure, int currentPlayer, int cardDrawn, int temphand[])
{
  ...
  while(z>=0){ //bug introdcued here "z-1>=0" changed to "z>=0"
...
int councilRoomEffect(struct gameState *state, int currentPlayer, int handPos)
{
  ...
  {
    if ( i == currentPlayer ) //bug introduced "i == currentPlayer" should read "i != currentPlayer"
      {
        drawCard(i, state);
      }
  }
...
int smithyEffect(int currentPlayer, int handPos, struct gameState *state)
{
  int i;
  //+3 cards
  for (i = 0; i <= 3; i++) //bug introduced here -- <= instead of <
  {
    drawCard(currentPlayer, state);
  }
...
int villageEffect(struct gameState *state, int currentPlayer, int handPos)
{
  ...
  //+2 Actions
  state->numActions = state->numActions++; //bug introduced here: ++ should be +2
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      return adventurerEffect(state, drawntreasure, currentPlayer, cardDrawn, temphand);
